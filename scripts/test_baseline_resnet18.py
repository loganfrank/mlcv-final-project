...
from networks.resnet18 import resnet18 as resnet
...
if __name__ == '__main__':
    ...
    num_epochs = 50
    experiment = f'resnet18_{dataset}_batchsize{batch_size}_lr{learning_rate}_numepochs{num_epochs}_2'
    ...
    transform = transforms.Compose([AdaptiveCenterCrop(), Resize(10), transforms.ToTensor(), transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    ...
    if balanced:
        ...
        test_dataset = datasets.balance_dataset(image_root_directory=image_directory, dataframe=dataframe,  
                                                transform=transform, phase='test', balance=True, cut=1)
        ...
        val_dataset = datasets.balance_dataset(image_root_directory=image_directory, dataframe=dataframe, 
                                                transform=transform, phase='val', balance=True, cut=1)
        val_dataloader = DataLoader(val_dataset, batch_size=1, shuffle=False)
    else:
        # Create the test dataset
        test_dataset = datasets.imbalance_dataset(image_root_directory=image_directory, dataframe=dataframe, transform=transform, phase='test')
        test_dataloader = DataLoader(test_dataset, batch_size=1, shuffle=False)

        # Create the validation dataset
        val_dataset = datasets.imbalance_dataset(image_root_directory=image_directory, dataframe=dataframe, transform=transform, phase='val')
        val_dataloader = DataLoader(val_dataset, batch_size=1, shuffle=False)
    ...
    evaluate_network.validate_baseline_network(network=network, dataloader=val_dataloader, compute_device=compute_device, 
                                                            experiment=f'{experiment}_bal{balanced}', results_directory=results_directory, 
    ...
    evaluate_network.test_baseline_network(network=network, dataloader=test_dataloader, compute_device=compute_device, 
                                        experiment=f'{experiment}_bal{balanced}', results_directory=results_directory, save_results=True)